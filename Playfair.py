...
if opt==1:
	...
	if (PlainTextList[-1][1]== '\n') :
		PlainTextList[-1] = PlainTextList[-1][0]+'z'
	key = toLowerCase(key)
	...
	for i in CipherList:
		CipherText += i
	print("\nCipher Text obtained is: ", CipherText)
	...
	print("\nDecrypted Text obtained is: ", decryptedText[:len(plainText)-1])

	ex= open("output_file.txt","w")
	ex.write("Cipher Text is: "+ CipherText)
	ex.write("\nDecrypted Text is: "+ decryptedText[:len(plainText)-1])
    ...
    print('Cipher text is "'+ encryptByPlayfairCipher(plainText , key)+'"')